...
/**
 * binary_tree_levelorder - Search through a bin-tree
...
void binary_tree_levelorder(const binary_tree_t *tree, void (*func)(int))
{
	levelorder_queue_t *dev_hd, *d_tail;

	if (tree == NULL || func == NULL)
		return;

	dev_hd = d_tail = create_node((binary_tree_t *)tree);
	if (dev_hd == NULL)
		return;

	while (dev_hd != NULL)
	{
		pint_push(dev_hd->node, dev_hd, &d_tail, func);
		pop(&dev_hd);
	}
}
